...
interface ImagePostCommentItemProps {
  comment: CommentData;
  imagePostId: string;
  parentId?: string;
  isReply?: boolean;
...
export function ImagePostCommentItem({
  comment,
  imagePostId,
  parentId,
  isReply = false,
  ...
  } = trpc.imagePostComment.getReplies.useInfiniteQuery(
  ...
  const reactMutation = trpc.imagePostComment.react.useMutation({
  ...
  const createReplyMutation = trpc.imagePostComment.create.useMutation({
    onSuccess: () => {
      setReplyContent("");
      setIsReplying(false);
      setReplyToUser(null);
      setShowReplies(true);
      utils.imagePostComment.getReplies.invalidate({ commentId: topLevelParentId });
  ...
  const updateMutation = trpc.imagePostComment.update.useMutation({
    onSuccess: () => {
      setIsEditing(false);
      utils.imagePostComment.list.invalidate({ imagePostId });
      utils.imagePostComment.getReplies.invalidate();
      toast.success("编辑成功");
    },
    onError: (error) => {
  ...
  const deleteMutation = trpc.imagePostComment.delete.useMutation({
    onSuccess: () => {
      utils.imagePostComment.list.invalidate({ imagePostId });
      utils.imagePostComment.getReplies.invalidate();
  ...
  const pinMutation = trpc.imagePostComment.pin.useMutation({
    onSuccess: () => {
      utils.imagePostComment.list.invalidate({ imagePostId });
  ...
  }, [replyContent, createReplyMutation, imagePostId, topLevelParentId, replyToUser, comment.user?.id]);
